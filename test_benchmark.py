...
F_START_PNTS = [(0, 0), (10, 10), (10, 100), (-10, -30), (1000, 1000)]

G_START_PNTS = [(x,)for x in [-1.7, -1, 0, 1, 1.2, 1.5, 1.8, 2]]

LEARN_COEFS = [1.2, 1, 0.8, 0.5, 0.1, 0.06]
...
def plot_steps_3d(
    ...
    xs = np.arange(xlim[0], xlim[1], (xlim[1]-xlim[0])/sqrt(GRAPH_RESOLUTION))
    ys = np.arange(ylim[0], ylim[1], (ylim[1]-ylim[0])/sqrt(GRAPH_RESOLUTION))
...
def label_steps(start_pnt: Tuple[float, ...], learn_coef: float,
    ...
    if len(start_pnt) > 1:
        start_pnt_str = f"x=({','.join(str(x) for x in start_pnt)})"
    else:
        start_pnt_str = f"x={start_pnt[0]}"
    learn_coef_str = f"a={learn_coef}"
    ...
    filename = f"{start_pnt_str},{learn_coef_str}.svg"
    return filename
...
def plot_performance_vs_learn_coef(
    ...
    ax.set_xlabel("Initial learning coefficient")
    ax.set_ylabel("Count")
...
if __name__ == "__main__":
    parser = ArgumentParser("Benchmark analyzer")
    parser.add_argument(
